    ...
    for sheet in excel_sheet:
        for column_name in excel_sheet[sheet]:
            ...
            if column_name in sports_name:
                ...
                sports[column_name]["teams"] = generate_teams(sport_config,
                                                              get_athletes(sport_votes, athletes))
        break
...
def parse_exported_excel(data_dir: str) -> None:
    ...
    for sport_name in sports_name:
        ...
        if sport_config["Type"] == "Table":
            ...
            series = generate_series([dict(Players="")]*(1 << (len(teams_list["Teams"]).bit_length() - 1)), sport_config, True)
        ...
        status_info = dict(
                           ...
                           states=states,
                           arbitre=[""],
                           rules=sport_config["rules"],
                           sportname=file_name.replace("_status.json", ""))
        with open(f"{data_dir}/teams/{file_name}", "w") as file:
